...
**AI assistants:** Read this file, then the relevant section for your task.

**New team members:** Read `01-company/` first, then `04-team/` and `08-sops/`.

**Developers:** Jump to `02-product/` for architecture and codebase maps.

**Before any major decision:** Check `05-roadmap/` to avoid duplicate or conflicting work.
...
AI reads the full vault before any task to build context. AI writes to `daily/` for session summaries, or updates relevant sections. AI never deletes `00-start-here.md` or the root structure. All AI edits are committed with descriptive messages. Use `[[wikilinks]]` for cross-referencing between notes.
